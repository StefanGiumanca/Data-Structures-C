...
struct Masina* copiazaPrimeleNElemente(struct Masina* vector, int nrElemente, int nrElementeCopiate) {
	...
	if (vector != NULL && nrElementeCopiate <= nrElemente && nrElementeCopiate > 0)
	{
		...
		{
			vectorNou[i] = initializare(vector[i].id, vector[i].anFabricatie, vector[i].sofer,
				vector[i].kilometriiParcursi, vector[i].initialaProducator);
		}
	}
	return vectorNou;
}
...
int main() {
	...
	vector[1] = initializare(2, 2014, "Stefan", 1500.6, 'M');
	vector[2] = initializare(3, 2017, "Cristi", 7500, 'A');
